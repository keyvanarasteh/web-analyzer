...
async fn test_scan_content() {
    ...
    assert!(info.summary.total_urls_crawled >= 1, "Should crawl at least 1 page");
...
fn test_shannon_entropy() {
    ...
    assert!(h_high > 3.5, "High entropy string should be > 3.5, got {}", h_high);
    assert!(h_low < 1.0, "Low entropy string should be < 1.0, got {}", h_low);
