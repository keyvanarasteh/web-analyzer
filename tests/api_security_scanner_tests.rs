...
mod api_scanner_tests {
    ...
    async fn test_scan_api_endpoints() {
        ...
        assert!(info.total_paths_probed > 100, "Should probe 800+ API paths, got {}", info.total_paths_probed);
        
        println!("Scan complete: probed {} paths, found {} endpoints, {} vulns, tested {}",
            info.total_paths_probed, info.endpoints_found.len(),
            info.vulnerabilities.len(), info.endpoints_tested);
    }
    ...
    fn test_ssti_expected_results() {
        ...
        for (payload, expected) in &tests {
            ...
            assert!(expected_num > 100, "Expected result {} should be > 100 to avoid false positives", expected);
            assert!(!payload.contains(expected), "Payload '{}' should not contain expected '{}'", payload, expected);
        }
    }
    ...
    fn test_auth_bypass_header_parsing() {
        ...
        assert!(header_names.iter().any(|n| n.contains("X-Forwarded-For") || n.contains("X-Real-IP")),
            "Should contain common bypass headers, got: {:?}", header_names);
    }
    ...
    fn test_payload_loading() {
        ...
        assert!(sqli.len() > 5, "Should have SQLi payloads, got {}", sqli.len());
        ...
        assert!(ssrf.len() > 3, "Should have SSRF payloads, got {}", ssrf.len());
        
        let cmd = payloads::lines(payloads::COMMAND_INJECTION);
        assert!(cmd.len() > 3, "Should have CMD injection payloads, got {}", cmd.len());
        
        let nosql = payloads::lines(payloads::NOSQL_INJECTION);
        assert!(nosql.len() > 2, "Should have NoSQL payloads, got {}", nosql.len());
    ...
    fn test_api_endpoints_payload() {
        ...
        assert!(endpoints.len() > 700, "Should have 700+ API endpoint patterns, got {}", endpoints.len());
        
        // Verify known endpoints exist
        assert!(endpoints.iter().any(|e| e.contains("/api")), "Should contain /api paths");
        assert!(endpoints.iter().any(|e| e.contains("/graphql")), "Should contain /graphql");
    }
    ...
    fn test_html_killer_filter() {
        ...
        assert!(html_killers.iter().any(|k| html_lower.contains(k)), 
        ...
        assert!(!html_killers.iter().any(|k| api_lower.contains(k)),
    ...
    fn test_api_structure_scoring() {
        ...
        for (content, should_match) in &test_cases {
            let score: usize = patterns.iter()
                .filter_map(|p| Regex::new(p).ok())
                .filter(|rx| rx.is_match(content))
                .count();
            
            if *should_match {
                assert!(score > 0, "Content '{}' should match at least one API pattern", content);
            } else {
                assert_eq!(score, 0, "Content '{}' should NOT match any API pattern", content);
            }
        }
    }
}
