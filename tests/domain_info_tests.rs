...
async fn test_get_domain_info_basic() {
    ...
    assert!(result.is_ok(), "Failed to get domain info: {:?}", result.err());
    ...
    assert!(!info.all_ipv4.is_empty(), "Should have at least one IPv4 in all_ipv4");
    
    // It should also have IPv6
    assert!(!info.ipv6.is_empty(), "Should have resolved an IPv6 address for example.com");
