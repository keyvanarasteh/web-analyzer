    ...
    if let Some(t) = &progress_tx {
        let _ = t
            .send(crate::ScanProgress {
                ...
                percentage: 5.0,
                message: "Initializing subdomain takeover checks...".into(),
                status: "Info".into(),
            })
            .await;
    }
    ...
    for (i, sub) in subdomains.iter().enumerate() {
        if let Some(t) = &progress_tx {
            let _ = t
                .send(crate::ScanProgress {
                    module: "Subdomain Takeover".into(),
                    percentage: 5.0 + (90.0 * (i as f32 / subdomains.len().max(1) as f32)),
                    message: format!("Checking {} for dangling records...", sub),
                    status: "Info".into(),
                })
                .await;
        }
    ...
    if let Some(t) = &progress_tx {
        let _ = t
            .send(crate::ScanProgress {
                module: "Subdomain Takeover".into(),
                percentage: 95.0,
                message: "Sorting and finalizing vulnerability results...".into(),
                status: "Info".into(),
            })
            .await;
    }
