...
impl WebAnalyzerMessageHandler {
    ...
    async fn dispatch(&self, event: &str, data: &Value) -> anyhow::Result<Value> {
        match event {
            ...
            "domain_info" => {
                ...
                if domain.is_empty() {
                    return Err(anyhow::anyhow!("Missing 'domain' parameter"));
                }
                let res = crate::domain_info::get_domain_info(&domain)
                    .await
            ...
            "domain_dns" => {
                ...
                let res = crate::domain_dns::get_dns_records(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "seo_analysis" => {
                ...
                let res = crate::seo_analysis::analyze_advanced_seo(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "web_technologies" => {
                ...
                let res = crate::web_technologies::detect_web_technologies(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "domain_validate_bulk" => {
                let domains: Vec<String> = data
                    ...
                    .unwrap_or_default();
                if domains.is_empty() {
                    return Err(anyhow::anyhow!("Missing 'domains' array parameter"));
                }
            ...
            "subdomain_discovery" => {
                ...
                let res = crate::subdomain_discovery::discover_subdomains(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "contact_spy" => {
                ...
                let max_pages: usize =
                    data.get("max_pages").and_then(|v| v.as_u64()).unwrap_or(20) as usize;
                let res = crate::contact_spy::crawl_contacts(&domain, max_pages)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "content_scan" => {
                ...
                let res = crate::advanced_content_scanner::scan_content(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "security_analysis" => {
                ...
                let res = crate::security_analysis::analyze_security(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "subdomain_takeover" => {
                ...
                let subdomains: Vec<String> = data
                    .get("subdomains")
                    .and_then(|v| serde_json::from_value(v.clone()).ok())
                    .unwrap_or_default();
                if subdomains.is_empty() {
                    return Err(anyhow::anyhow!("Missing 'subdomains' array parameter"));
                }
                let res = crate::subdomain_takeover::check_subdomain_takeover(&domain, &subdomains)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "cloudflare_bypass" => {
                ...
                let res = crate::cloudflare_bypass::find_real_ip(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "nmap_scan" => {
                ...
                let res = crate::nmap_zero_day::run_nmap_scan(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "api_security_scan" => {
                ...
                let res = crate::api_security_scanner::scan_api_endpoints(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            ...
            "geo_analysis" => {
                ...
                let res = crate::geo_analysis::analyze_geo(&domain)
                    .await
                    .map_err(|e| anyhow::anyhow!("{:?}", e))?;
                Ok(serde_json::to_value(res)?)
            }
            _ => Err(anyhow::anyhow!(
                "Unknown web-analyzer event or feature disabled: {}",
                event
            )),
        }
    }
...
impl WsMessageHandler for WebAnalyzerMessageHandler {
    async fn handle_text(
        ...
        let msg: Value = match serde_json::from_str(text) {
            ...
            Err(_) => {
                return self
                    .fallback
                    .handle_text(addr, text, server, user_id, browser_id)
                    .await
            }
        };
        ...
        let _ = server
            .send_to(addr, Message::Text(response.to_string().into()))
            .await;
        Ok(true)
    }
    ...
    async fn handle_binary(
        ...
        self.fallback
            .handle_binary(addr, data, server, user_id, browser_id)
            .await
    }
